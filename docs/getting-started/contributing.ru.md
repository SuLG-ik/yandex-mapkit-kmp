...
В `ymk-docs/` лежит офлайн-копия справочника Яндекс MapKit — источник истины по сигнатурам, которые
повторяет обёртка. Копия генерируется и в репозитории не хранится, поэтому после клонирования её
нужно собрать:
...
Скрипт скачивает около 2000 страниц с `yandex.ru/maps-api/docs/mapkit` и конвертирует их в Markdown —
это пара минут. Он идемпотентен: если копия уже собрана, ничего не делает. `--validate` добавляет
проверки покрытия и ссылок, `--force` пересобирает заново, `--refresh` качает страницы вместо того,
чтобы брать кеш из `ymk-docs/_tools/cache/`.
