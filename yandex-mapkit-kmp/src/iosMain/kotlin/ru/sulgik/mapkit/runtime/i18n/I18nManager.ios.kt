...
import YandexMapKit.YRTI18nManager as NativeI18nManager
import YandexMapKit.YRTI18nManagerFactory as NativeI18nManagerFactory
...
public actual class I18nManager internal constructor(private val nativeI18nManager: NativeI18nManager) {
    ...
    public actual fun localizeDistance(distance: Int): String {
        return nativeI18nManager.localizeDistanceWithDistance(distance.toLong())
    }

    public actual fun localizeDuration(duration: Int): String {
        return nativeI18nManager.localizeDurationWithDuration(duration.toLong())
    }

    public actual fun localizeSpeed(speed: Double): String {
        return nativeI18nManager.localizeSpeedWithSpeed(speed)
    }

    public actual fun localizeDataSize(dataSize: Long): String {
        return nativeI18nManager.localizeDataSizeWithDataSize(dataSize)
    }

    public actual fun canonicalSpeed(speed: Double): CanonicalUnit {
        return nativeI18nManager.canonicalSpeedWithSpeed(speed).toCommon()
    }

    public actual fun localizeCanonicalUnit(canonicalUnit: CanonicalUnit): String {
        return nativeI18nManager.localizeCanonicalUnitWithCanonicalUnit(canonicalUnit.toNative())
    }
    ...
    public actual companion object {
        ...
        public actual fun setLocale(locale: String?) {
            NativeI18nManagerFactory.setLocaleWithLocale(locale)
        }
