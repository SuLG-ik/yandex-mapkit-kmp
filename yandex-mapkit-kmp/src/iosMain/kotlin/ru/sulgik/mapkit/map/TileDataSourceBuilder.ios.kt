...
import YandexMapKit.YMKTileDataSourceBuilder as NativeTileDataSourceBuilder
...
public actual class TileDataSourceBuilder internal constructor(
    ...
    public actual fun setTileUrlProvider(urlProvider: UrlProvider) {
        nativeBuilder.setTileUrlProviderWithUrlProvider(urlProvider.toNative())
    }
    ...
    public actual fun setTileProvider(tileProvider: TileProvider) {
        nativeBuilder.setTileProviderWithTileProvider(tileProvider.toNative())
    }

    public actual fun setProjection(projection: Projection) {
        nativeBuilder.setProjectionWithProjection(projection.toNative())
    }

    public actual fun setZoomRanges(zoomRanges: List<ZoomRange>) {
        nativeBuilder.setZoomRangesWithZoomRanges(zoomRanges.map { it.toNative() })
    }

    public actual fun setTileFormat(format: TileFormat) {
        nativeBuilder.setTileFormatWithFormat(format.toNative())
    }
